...
\end{lstlisting}
The App.js file handles the app structure; it is primarily a set of router fragments that link to each primary component of the web application. Because the user is being tracked on a leaderboard, users can only reach the Wordle component if logged in. The solution is that the useState is created inside the app file; users only have access to signup, login, and leaderboard if they are not currently logged in. This is done using ternary operators, which only run the router fragment if the user has been validated. Instead, users will reach the final router fragment, which, depending on whether they are logged in, will route them to Wordle or the login page. 

\subsection{JWT}
When creating the Wordle app, it became apparent that there would need to be some sort of verification between the front and back end that the user that was set into the user useState was validated. After some research, JSON Web Tokens were implemented into the website.\cite{jwt2024} With this, the payload would be the username encoded using a public/private key pair and stored in the user's local storage. Whenever a backend function call is made, the JWT is sent to the headers and decoded in the back end to verify the correct user. The username is then pulled from the JWT and used in many function calls to draw the correct user from the database.
\begin{figure}[h] % 'h' means 'here' - places the figure where it's defined
    \centering
...
\subsection{Keyboard}
During the early stages of the web app, when only the game's logic was in place, it became clear how vital the keyboard was to the “game feel” of Wordle. The keyboard not only gives users the ability to type in their guesses but also helps them keep track of the game's current state. When users have an idea for their next guess, they refer to the keyboard to see which letters they have already tried. Without the keyboard, a user has to refer back to their guesses, which is frustrating and challenging to keep track of. 
The keyboard is relatively simple to implement; it passes information into a child component, such as the letter it should submit, into the input box. It also passes the state of the key to the element, so the colour is updated for each guess. 
...
\end{lstlisting}
WordleN is a part of the efforts to increase the functionality of the Wordle application. In this component, the user can set a different word length of six, seven, or eight-letter words to increase the level of challenge. The logic is, of course, very similar to the main Wordle component. However, during personal testing in the development phase, it became clear that six guesses to find the solution of an eight-letter word would be too challenging for users. The solution is that users get an extra guess with each additional letter they must guess, meaning that eight-letter words give users nine guesses. Fortunately, the already implemented code for Wordle accounted for this and will still display guesses until the loss useState is updated.

\subsection{Navbar}
For ease of use, a navbar is implemented on each application webpage, which also uses the router function. The main challenges faced when implementing the navbar were only showing links to the pages that the user had access to and having an active breadcrumb to show which page was currently active. 
The solution to the first issue was similar to that in the app structure. The Navbar uses ternary operators based on whether the user is logged in. Even if the navbar still displayed the links to other application pages, users would be redirected if they were not logged in. However, for clarity, it is critical to display to the user the parts they can use depending on whether they are logged in. 
...
When a user has reached a finished state in the game, whether they have won or not, a modal box is displayed asking if they wish to play again. If they do, the page does not reload; instead, it resets all the useStates within the react component and clears the table of guesses. This modal box also allows users to share the word they have just tried to guess and send it to another user. 
...
\end{lstlisting}
To accommodate share functionality, a route is created in a router fragment within the app structure. This will redirect to Wordle or WordleN, depending on which page users got the share link from. The modal box at the end of each game has a share button. When this is clicked, a request is sent to the backend, and with this, the frontend sends the answer. A UUID is then returned to the front end, and the body of the link is created when a user clicks on it.

\subsection{Share Backend}
The backend generates a UUID and creates a game in the SharedGame table in the database. This table contains a UUID and an answer. When this link is used, the UUID will be pulled from the link and checked against the SharedGame table. What is done after this is the row with that UUID is found, and the answer is then sent to the createGame function. When a game is returned to the front end, it will function the same, but the answer will now be the word from the game that the user sent it from.
